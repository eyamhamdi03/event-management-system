...
const DUMMY_TOKEN = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiI2YWFmMTBiYi01NTFmLTQ5NjgtODg5Mi1kNGFhMzgxMWIwNjgiLCJmdWxsTmFtZSI6IkV2ZW50IE9yZ2FuaXplciIsImVtYWlsIjoib3JnYW5pemVyQGV4YW1wbGUuY29tIiwicm9sZSI6Im9yZ2FuaXplciIsImlhdCI6MTc0ODc0MjgzNSwiZXhwIjoxNzQ4NzQzNzM1fQ.ylfDZdmw3FGVaNwU8MKTp6YYo3IyjY-ltzgSRo230eI'
...
function OrganizerEventsPage() {
  ...
  useEffect(() => {
    async function loadEvents() {
      setLoading(true)
      try {
        const result = await fetchEventsByOrganizer(organizerId, DUMMY_TOKEN)
  ...
  async function loadCategories() {
      try {
        const cats = await fetchCategories(DUMMY_TOKEN)
        setCategories(cats)
      } catch (e) {
 ...
 const handleAdd = async (eventData: any) => {
  await createEvent({ ...eventData, organizerId }, DUMMY_TOKEN)
  const result = await fetchEventsByOrganizer(organizerId, DUMMY_TOKEN)
  const eventsRaw = Array.isArray(result?.data) ? result.data : []
  setEvents(eventsRaw)
  const organizer = eventsRaw.length > 0 ? eventsRaw[0].host?.fullName || '' : ''
...
const handleDelete = async (id: string) => {
  await deleteEvent(id, DUMMY_TOKEN)
  const result = await fetchEventsByOrganizer(organizerId, DUMMY_TOKEN)
  const eventsRaw = Array.isArray(result?.data) ? result.data : []
  setEvents(eventsRaw)
  const organizer = eventsRaw.length > 0 ? eventsRaw[0].host?.fullName || '' : ''
